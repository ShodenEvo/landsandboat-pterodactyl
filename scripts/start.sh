...
def sub(t,p,r,n):
    ...
    return t
ipaddress.ip_address(os.environ["PUBLIC_SERVER_IP"])
...
q=c.cursor(); q.execute("UPDATE zone_settings SET zoneip=?,zoneport=? WHERE zoneport<>0",(os.environ["PUBLIC_SERVER_IP"],int(os.environ["MAP_PORT"]))); c.commit(); q.close(); c.close()
PYDB
