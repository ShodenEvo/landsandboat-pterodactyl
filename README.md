...
1. Open **Nests**.
2. Create or select a Nest for FINAL FANTASY XI.
3. Choose **Import Egg**.
4. Upload `egg-final-fantasy-xi-landsandboat.json`.
5. Create a new server using the imported Egg.
6. Select `landsandboat-pterodactyl:latest` as the runtime image.
...
| Purpose | Port |
|---|---:|
| Login configuration | `51220` |
| Login view | `54001` |
| Search | `54002` |
| Map and zone traffic | `54230` |
| Xiloader authentication | `54231` |
...
| `DB_NAME` | LandSandBoat database name |
| `DB_USER` | Dedicated LandSandBoat database user |
| `DB_PASSWORD` | Dedicated database password |
| `PUBLIC_SERVER_IP` | Address advertised to game clients |
| `VER_LOCK` | Client-version policy |
...
| Value | Behaviour |
|---:|---|
| `0` | Allow all client versions |
| `1` | Require the exact configured client version |
| `2` | Allow the configured version or newer |
...
3. Back up custom Lua files and modules.
4. Update the selected branch, tag, or commit.
5. Rebuild the binaries.
